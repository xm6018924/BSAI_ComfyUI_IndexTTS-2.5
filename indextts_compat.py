...
# ===========================================================================
# 11. Also inject model_parallel_utils into the top-level transformers.utils
#     since some code does `from transformers.utils.model_parallel_utils import ...`
# ===========================================================================
# Already done above

# ===========================================================================
# 12. pytorch_utils: prune_layer
# ===========================================================================
if not hasattr(_pytorch_utils, 'prune_layer'):
...
if not hasattr(_tf_utils, 'download_url'):
    def download_url(url, proxies=None):
        """Download a URL (from transformers 4.x)."""
...
if not hasattr(_tf_utils, 'is_offline_mode'):
    def is_offline_mode():
        """Check if transformers is in offline mode."""
...
if not hasattr(_tf_utils, 'is_torch_sdpa_available'):
    def is_torch_sdpa_available():
        """Check if torch SDPA is available."""
        try:
...
# ===========================================================================
# 14. modeling_flax_pytorch_utils (entire module missing - dummy)
# ===========================================================================
if 'transformers.modeling_flax_pytorch_utils' not in sys.modules:
    ...
    def load_flax_checkpoint_in_pytorch_model(model, flax_checkpoint_path, allow_pickle=False):
        """Dummy: Flax not available on this platform."""
...
# ===========================================================================
# 15. modeling_tf_pytorch_utils (entire module missing - dummy)
# ===========================================================================
if 'transformers.modeling_tf_pytorch_utils' not in sys.modules:
    ...
    def load_tf2_checkpoint_in_pytorch_model(model, tf_checkpoint_path, config=None, allow_pickle=False):
        """Dummy: TensorFlow not available on this platform."""
...
# ===========================================================================
# 16. Also check for transformers.utils items that might still be missing
# ===========================================================================
...
if not hasattr(_tf_utils, 'ContextManagers'):
    ...
    class ContextManagers:
        """A stack of context managers (from transformers 4.x)."""
...
if not hasattr(_tf_utils, 'PushToHubMixin'):
    class PushToHubMixin:
        """Dummy PushToHubMixin (from transformers 4.x)."""
...
if not hasattr(_tf_utils, 'copy_func'):
    ...
    def copy_func(f):
        """Copy a function (from transformers 4.x)."""
        g = _types.FunctionType(f.__code__, f.__globals__, name=f.__name__,
...
if not hasattr(_tf_utils, 'extract_commit_hash'):
    def extract_commit_hash(resolved_file, commit_hash):
        """Extract commit hash (from transformers 4.x)."""
        return commit_hash
...
if not hasattr(_tf_utils, 'has_file'):
    def has_file(path_or_repo, filename, revision=None, proxy=None, token=None):
        """Check if a file exists (from transformers 4.x)."""
        import os
...
if not hasattr(_tf_utils, 'replace_return_docstrings'):
    def replace_return_docstrings(output_type=None, config_class=None):
        """Dummy decorator (from transformers 4.x)."""
...
if not hasattr(_tf_utils, 'strtobool'):
    def strtobool(value):
        """Convert string to boolean (from transformers 4.x / distutils)."""
        value = value.lower()
...
try:
    ...
    if not hasattr(_hub, 'create_and_tag_model_card'):
        def create_and_tag_model_card(*args, **kwargs):
            return None
        _hub.create_and_tag_model_card = create_and_tag_model_card
    if not hasattr(_hub, 'get_checkpoint_shard_files'):
        def get_checkpoint_shard_files(*args, **kwargs):
            return None, None
        _hub.get_checkpoint_shard_files = get_checkpoint_shard_files
except Exception:
    pass

# ===========================================================================
# 23. utils.quantization_config
# ===========================================================================
try:
